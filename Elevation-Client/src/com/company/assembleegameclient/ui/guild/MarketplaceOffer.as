...
import com.company.assembleegameclient.ui.DeprecatedTextButton;
    ...
    public class MarketplaceOffer extends Sprite {
        ...
        private var tradeButton_:DeprecatedTextButton;
        private var id_:int;


        public function MarketplaceOffer(_arg1:XML, _arg2:AGameSprite) {
            super();
        ...
        private function draw():void {
            ...
            this.tradeButton_ = new DeprecatedTextButton(12, _local3);
            this.tradeButton_.x = (((3 * MemberListLine.WIDTH) / 3.25) - (this.tradeButton_.width / 2));
            this.tradeButton_.y = (HEIGHT / 2) - (this.tradeButton_.height / 2);
            this.tradeButton_.setEnabled(false);
            this.usableCheck();
        ...
        private function usableCheck():void {
            if (!this.info_.mine_) {
                var _pItems:Vector.<int> = this.gs_.map.player_.equipment_;
                var _usedItems:Vector.<int> = new Vector.<int>();
               var _pData:Vector.<Object> = this.gs_.map.player_.equipData_;
                for (var i:int = 0; i < this.info_.requestItems_.length; i++) {
                    ...
                    for (var p:int = 0; p < _pItems.length; p++) {
                        ...
                        var _xml:XML = ObjectLibrary.xmlLibrary_[_pItems[p]];
                        if (_xml.hasOwnProperty("Soulbound")/* && _pData.Soulbound*/) {
                            continue;
                        }
                        _usedItems.push(p);
                        _success = true;
                ...
                if (_availableSlots < this.info_.offerItems_.length) {
                    ...
                    this.tradeButton_.setEnabled(true);
                }
            } else if (!this.info_.status_) {
                this.tradeButton_.setEnabled(true);
            } else {
                var available:int = 0;
                for(var i:int = 4; i < this.gs_.map.player_.equipment_.length; i++) {
                    if(this.gs_.map.player_.equipment_[i] == -1) {
                        available++;
                    }
                }
                var maxLen:int = 0;
