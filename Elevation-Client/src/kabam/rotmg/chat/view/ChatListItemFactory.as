...
public class ChatListItemFactory {
    ...
    private function processName():String {
        ...
        if (_local_1.indexOf("%color%") >= 0)
            _local_1 = _local_1.substr(_local_1.indexOf("%endcolor%") + 10);
        return ((("<" + _local_1) + ">"));
    }
    ...
    private function getNameColor():uint {
        if (this.message.name.indexOf("%color%") >= 0) {
            var _array:Array = this.message.name.split("%color%");
            for each (var i:String in _array) {
                var color:String = i.substr(0, i.indexOf("%endcolor%"));
                i = i.substr(i.indexOf("%endcolor%") + 10);
            }
            return uint(color);
        }
