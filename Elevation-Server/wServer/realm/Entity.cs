...
namespace wServer.realm
{
    public class Entity : IProjectileOwner, ICollidable<Entity>, IDisposable
    {
        ...
        {
            if (Owner != null && !(this is Projectile) && (!(this is StaticObject) || ((StaticObject)this).Hittestable))
                (this is Enemy ? Owner.EnemiesCollision : Owner.PlayersCollision).Move(this, x, y);
            if (X != x || Y != y)
                UpdateCount++;
            X = x;
            Y = y;
        ...
        public Projectile CreateProjectile(ProjectileDesc desc, ushort container, int dmg, Position pos,
            float angle)
        {
