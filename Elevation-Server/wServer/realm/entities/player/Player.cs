...
namespace wServer.realm.entities.player
{
    ...
    public partial class Player : Character, IContainer, IPlayer
    {
		...
		private void CheckCurrencyAmounts()
        {
            var types = new string[3] { "Bronze", "Silver", "Gold" };

            for (var h = 0; h < 3; h++)
            {
                if (Inventory.Where(_ => _ != null && _.ObjectId == $"1 {types[h]}").Count() == 5)
                {
                    var x = 5;
                    for (var i = 4; i < Inventory.Length; i++)
                        if (x != 0 && Inventory[i] != null && Inventory[i].ObjectId == $"1 {types[h]}")
                        {
                            Inventory[i] = null;
                            x--;
                        }
                    ...
                    for (var i = 4; i < Inventory.Length; i++)
                        if (Inventory[i] == null)
                        {
                            Inventory[i] = Manager.GameData.Items[Manager.GameData.IdToObjectType[$"5 {types[h]}"]];
                            UpdateCount++;
                            break;
                        }
                }

                if (Inventory.Where(_ => _ != null && _.ObjectId == $"5 {types[h]}").Count() == 2)
                {
                    var x = 2;
                    for (var i = 4; i < Inventory.Length; i++)
                        if (x != 0 && Inventory[i] != null && Inventory[i].ObjectId == $"5 {types[h]}")
                        {
                            Inventory[i] = null;
                            x--;
                        }
                    ...
                    for (var i = 4; i < Inventory.Length; i++)
                        if (Inventory[i] == null)
                        {
                            Inventory[i] = Manager.GameData.Items[Manager.GameData.IdToObjectType[$"10 {types[h]}"]];
                            UpdateCount++;
                            break;
                        }
                }

                if (Inventory.Where(_ => _ != null && _.ObjectId == $"5 {types[h]}").Count() == 1 && Inventory.Where(_ => _ != null && _.ObjectId == $"10 {types[h]}").Count() == 2)
                {
                    var x = 1;
                    for (var i = 4; i < Inventory.Length; i++)
                        if (x != 0 && Inventory[i] != null && Inventory[i].ObjectId == $"5 {types[h]}")
                    ...
                    x = 2;
                    for (var i = 4; i < Inventory.Length; i++)
                        if (x != 0 && Inventory[i] != null && Inventory[i].ObjectId == $"10 {types[h]}")
                        {
                            Inventory[i] = null;
                            x--;
                        }
                    ...
                    for (var i = 4; i < Inventory.Length; i++)
                        if (Inventory[i] == null)
                        {
                            Inventory[i] = Manager.GameData.Items[Manager.GameData.IdToObjectType[$"25 {types[h]}"]];
                            UpdateCount++;
                            break;
                        }
                }

                if (Inventory.Where(_ => _ != null && _.ObjectId == $"25 {types[h]}").Count() == 2)
                {
                    var x = 2;
                    for (var i = 4; i < Inventory.Length; i++)
                        if (x != 0 && Inventory[i] != null && Inventory[i].ObjectId == $"25 {types[h]}")
                        {
                            Inventory[i] = null;
                            x--;
                        }
                    ...
                    for (var i = 4; i < Inventory.Length; i++)
                        if (Inventory[i] == null)
                        {
                            Inventory[i] = Manager.GameData.Items[Manager.GameData.IdToObjectType[$"50 {types[h]}"]];
                            UpdateCount++;
                            break;
                        }
                }
                ...
                if(types[h] != "Gold")
                    if (Inventory.Where(_ => _ != null && _.ObjectId == $"50 {types[h]}").Count() == 2)
                    {
                        var x = 2;
                        for (var i = 4; i < Inventory.Length; i++)
                            if (x != 0 && Inventory[i] != null && Inventory[i].ObjectId == $"50 {types[h]}")
                            {
                                Inventory[i] = null;
                                UpdateCount++;
                                x--;
                        ...
                        for (var i = 4; i < Inventory.Length; i++)
                            if (Inventory[i] == null)
                            {
                                Inventory[i] = Manager.GameData.Items[Manager.GameData.IdToObjectType[$"1 {types[h + 1]}"]];
                                UpdateCount++;
                                break;
                            }
                    }
