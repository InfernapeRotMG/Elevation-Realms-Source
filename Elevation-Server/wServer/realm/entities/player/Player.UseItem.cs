                    ...
                    case ActivateEffects.LootBox:
                        {
                            ...
                            for (var i = 4; i < Inventory.Length; i++)
                                if (Inventory[i] == null)
                                {
                                    Inventory[i] = Manager.GameData.Items[eff.Rewards[Random.Next(0, eff.Rewards.Count)]];
                                    UpdateCount++;
                                    SaveToCharacter();
                                    return false;
                                }
                            SendHelp("Your inventory need free space to use this item.");
                            return true;
                        }
