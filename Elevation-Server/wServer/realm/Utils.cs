...
namespace wServer.realm
{
    internal static class EntityUtils
    {
        ...
        {
            var ret = spd * entity.Manager.TPS;
        ...
        {
            if (entity.Owner == null || entity.HasConditionEffect(ConditionEffectIndex.Paralyzed)) return false;

            if (x < 0 || x >= entity.Owner.Map.Width || y < 0 || y >= entity.Owner.Map.Height)
                return false;

            if (!entity.Owner.IsPassable((int) x, (int) y))
                return false;
