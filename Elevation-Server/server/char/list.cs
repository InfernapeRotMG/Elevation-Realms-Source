    ...
    internal class list : RequestHandler
    {
        ...
        protected override void HandleRequest()
        {
            using (Database db = new Database())
            {
                ...
                {
                    chrs.Account = new Account()
                    {
                        Name = "Registery Required",
                        AccountId = "0",
                        Admin = false,
                        Donator = false,
                        Banned = false,
                ...
                var p = GetLatLong(Context.Request.RemoteEndPoint.Address);
                if (p != null)
                {
                ...
                chrs.ClassAvailabilityList = GetClassAvailability(chrs.Account);
                var serializer = new XmlSerializer(chrs.GetType(), new XmlRootAttribute(chrs.GetType().Name) {Namespace = ""});
        ...
        List<ServerItem> GetServerList()
        {
            ...
            for (var i = 0; i < num; i++)
            {
                var p = GetLatLong(Program.Settings.GetValue<string>("svr" + i + "Location", ""));
                //var usage = GetUsage(Program.Settings.GetValue<string>("svr" + i + "Adr"));
        ...
        private static double GetUsage(string host, int port = 2050)
        {
            try
            {
                using (var cli = new TcpClient(host, 2050))
                {
                    ...
                    Array.Resize(ref buffer, cli.Client.Receive(buffer));
        ...
        private List<ClassAvailabilityItem> GetClassAvailability(Account acc)
        {
            ...
            if (acc == null)
            {
                return new List<ClassAvailabilityItem>
                {
                    ...
                    new ClassAvailabilityItem {Class = "Spartan", Restricted = "restricted"},
                };
            }
        ...
        public Chars GetChars(string guid, string password, XmlData data)
        {
            using (var db = new Database())
            {
                var a = db.Verify(guid, password, data);
                if (a != null)
                {
                    if (a.Banned)
                        return null;
