    ...
    internal class register : RequestHandler
    {
        protected override void HandleRequest()
        {
            ...
            using (var db = new Database())
            {
                ...
                if (db.HasUuid(Query["guid"]) &&  db.Verify(Query["guid"], "", Program.GameData) != null)
                {
                    ...
                    else
                    {
                        ...
                        cmd.CommandText =
                            "UPDATE accounts SET uuid=@newUuid, name=@newUuid, password=SHA1(@password) WHERE uuid=@uuid, name=@name;";
                    ...
                    status = Encoding.UTF8.GetBytes(db.Register(Query["newGUID"], Query["newPassword"], Program.GameData) != null ? "<Success/>" : "<Error>Error.emailAlreadyUsed</Error>");
