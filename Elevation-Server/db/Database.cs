...
AND characters.charId=death.chrId;";
        ...
        public Account Register(string uuid, string password, XmlData data)
        {
            MySqlCommand cmd = CreateQuery();
            ...
            cmd = CreateQuery();
            cmd.CommandText =
                "INSERT INTO accounts(uuid, password, name, admin, donator, namechosen, verified, guild, guildRank, guildFame, vaultCount, maxCharSlot, regTime, banned, locked, ignored, gifts, isAgeVerified, authToken) VALUES(@uuid, SHA1(@password), @randomName, 0, 0, 0, 1, 0, 0, 0, 1, 2, @regTime, 0, @empty, @empty, @empty, 1, @authToken);";
            cmd.Parameters.AddWithValue("@uuid", uuid);
        ...
        public Account GetAccount(string accId, XmlData data, string uuid=null, string password=null)
        {
            ...
            using (var rdr = cmd.ExecuteReader())
            {
                ...
                ret = new Account
                {
                    ...
                    Donator = rdr.GetBoolean("donator"),
...
SELECT MAX(chestId) FROM vaults WHERE accId = @accId;";
        ...
        public static Char CreateCharacter(XmlData data, ushort type, int chrId)
        {
            ...
            Char ret = new Char
            {
                ...
                Might = int.Parse(cls.Element("Might").Value),
...
VALUES(@accId, @chrId, @name, @objType, @tex1, @tex2, @skin, @items, @fame, @exp, @fameStats, @totalFame, @firstBorn, @killer);";
        ...
        public bool SaveChars(string oldAccId, Chars oldChars, Chars chrs, XmlData data)
        {
            try
            {
                ...
                var cmd = CreateQuery();
                cmd.CommandText = "UPDATE accounts SET name=@name, namechosen=@nameChoosen, vaultCount=@vaults, maxCharSlot=@maxChars, ownedSkins=@skins, gifts=@gifts WHERE id=@oldAccId;";
...
bestFame = GREATEST(bestFame, @bestFame);";
                ...
                foreach (var @char in chrs.Characters)
                {
                    ...
                    cmd.CommandText =
                        "INSERT INTO characters (accId, charId, charType, level, exp, fame, items, hp, mp, stats, dead, fameStats, skin) VALUES (@accId, @charId, @charType, 1, 0, 0, @items, 100, 100, @stats, FALSE, @fameStats, @skin);";
                    cmd.ExecuteNonQuery();
